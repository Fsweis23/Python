...
class Party:
    ...
    def get_by_id(cls, data):
        query = "SELECT * FROM parties JOIN users on parties.users.id WHERE parties.id = %(id)s;"
    ...
    @classmethod
    def update(cls, data):
        query = "UPDATE parties SET parties what = %(what)s, %(location)s, %(date)s, %(all_ages)s, %(description)s"\
            "WHERE id = %(id)s"
        return connectToMySQL(DATABASE).query_db(query, data)
