...
class Cars:
    ...
    def get_by_id(cls, data):
        ...
        this_car.planner = planner
        return this_car
    
    @classmethod
    def update(cls, data):
        query = "UPDATE cars SET price = %(price)s, model = %(model)s, make = %(make)s, year = %(year)s, description = %(description)s WHERE id = %(id)s AND user_id = %(user_id)s"
        return connectToMySQL(DATABASE).query_db(query, data)
